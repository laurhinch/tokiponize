// Merge kana-only Japanese aliases into wikidata-tok.jsonl. Kanji labels
// carry no phonetics, but the reading often exists as an alias (にほん).
...
for (let i = 0; i < ids.length; i += 50) {
  ...
  for (const [id, ent] of Object.entries(data.entities ?? {})) {
    const aliases = (ent.aliases?.ja ?? [])
      .map((a) => a.value)
      .filter((v) => KANA_ONLY.test(v));
    const row = byId.get(id);
    for (const [k, v] of [...new Set(aliases)].slice(0, 3).entries()) {
      row.labels[`ja-kana-${k}`] = v;
      added++;
    }
  }
